...
import { Message, createUserMessage, createAssistantMessage } from '../types/message.js';
...
describe('HistoryManager', () => {
  ...
  describe('getHistory', () => {
    ...
    it('should fetch from D1 if not in Redis cache', async () => {
      ...
      vi.spyOn(redisClient, 'lrange').mockResolvedValue([]);
      vi.spyOn(d1Client, 'query').mockResolvedValue(messages.map((m, i) => ({
        message_id: `msg${i}`,
    ...
    it('should limit results when specified', async () => {
      ...
      expect(redisClient.lrange).toHaveBeenCalledWith(`history:${sessionId}`, -5, -1);
    });
  });
  ...
  describe('getHistoryPage', () => {
    it('should return paginated results', async () => {
      ...
      vi.spyOn(d1Client, 'query').mockResolvedValue(messages.slice(0, 5));

      const result = await historyManager.getHistoryPage(sessionId, 5, 'cursor123');
    ...
    it('should handle empty results', async () => {
      ...
      const result = await historyManager.getHistoryPage('session123', 5, 'cursor123');
  ...
  describe('trimHistory', () => {
    it('should trim history to specified length', async () => {
      const sessionId = 'session123';
      const maxLength = 100;

      const result = await historyManager.trimHistory(sessionId, maxLength);

      expect(result).toBe(true);
      expect(redisClient.ltrim).toHaveBeenCalledWith(`history:${sessionId}`, -maxLength, -1);
    });

    it('should handle Redis failure', async () => {
      vi.spyOn(redisClient, 'ltrim').mockResolvedValue(false);

      const result = await historyManager.trimHistory('session123', 100);
  ...
  describe('getHistorySize', () => {
    it('should return history size from Redis', async () => {
      const sessionId = 'session123';
      vi.spyOn(redisClient, 'lrange').mockResolvedValue(['msg1', 'msg2', 'msg3']);

      const result = await historyManager.getHistorySize(sessionId);
    ...
    it('should return 0 for empty history', async () => {
      ...
      const result = await historyManager.getHistorySize('session123');
  ...
  describe('error handling', () => {
    ...
    it('should handle Redis connection errors', async () => {
      ...
      expect(result).toBe(true); // Should still succeed with D1 fallback
    });
  });
});